...
public interface MovieRepository  extends JpaRepository<Movie, Integer > {
    ...
    //native Query
    @Query(value = "SELECT * FROM movie m WHERE LOWER(m.title) LIKE LOWER(CONCAT('%', :title, '%'))", nativeQuery = true)
    ...
    //nativquery
    @Query(value = "SELECT CASE WHEN COUNT(*) > 0 THEN TRUE ELSE FALSE END FROM Movie m WHERE LOWER(m.title) = LOWER(:title)", nativeQuery = true)
    ...
    //native query
    @Query(value = "SELECT COUNT(m) FROM Movie m  WHERE LOWER(m.title) = LOWER(:title) ", nativeQuery = true)
    ...
    //Native ẻuery
    @Query(value = "SELECT * FROM Movie m WHERE m.status = :status AND m.type = :type", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM Movie m ORDER BY m.view desc ", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM Movie m WHERE m.status = :status ORDER BY :#{#sort.toJpaSort()}", nativeQuery = true)
    ...
    //Native query
    @Query(value = "SELECT * FROM Movie m WHERE m.type = :type AND m.status = :status ", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM Movie m WHERE m.type = :type AND m.id = :id AND m.slug = :slug", nativeQuery = true)
    ...
    //NQ
    @Query(value = "SELECT * FROM Movie m WHERE m.type = :type", nativeQuery = true)
